...
async def is_expired(token: str) -> bool | None:
    ...
    try:
        ...
        if int(datetime.datetime.now().timestamp()) < exp:
            return False
        #Qui ho solo ivertito il "True" con il "False" perché ho provato a gestire la sessione
        #da frontend ed ho notato che terminava subito quindi ho invertito ed ora dovrebbe funzionare
        return True
    except Exception as e:
        return None
